...
window.pages.goal = async function renderGoal(selectedTripId) {
  ...
  app.querySelectorAll(".trip-switcher .chip[data-id]").forEach((pill) => {
    pill.addEventListener("click", () => window.pages.goal(Number(pill.dataset.id)));
  });

  document.getElementById("add-saving-btn").addEventListener("click", () => {
    openSavingModal(activeTrip.id, { onSaved: () => window.pages.goal(activeTrip.id) });
  });

  document.getElementById("edit-trip-btn").addEventListener("click", () => {
    openTripEditModal(activeTrip, { onSaved: () => window.pages.goal(activeTrip.id) });
  });
